    ...
    function homeCtrl($scope, $filter, $http, $window, $state, $rootScope, $translate) {
        ...
        $scope.submitForm = function () {
            ...
            if ($scope.formData.dates.length > 0 && $scope.formData.title && $scope.formData.name && $scope.formData.email && !$scope.checkDuplicates()) {
                ...
                for (let i = 0; i < $scope.formData.dates.length; i++) {
                    let length = $scope.formData.dates[i].time.length;
                ...
                let parameter = JSON.stringify($scope.formData);
                console.log(parameter);

                $http.post("/createPoll", parameter).success(function (data) {
        ...
        $scope.$watch('formData.dt', function(newValue, oldValue) {
            ...
            if (newValue !== oldValue) {
                ...
                let formattedDate = datefilter($scope.formData.dt, 'yyyy/MM/dd');
                ...
                if (!isDateInDates(formattedDate)){
                    ...
                    obj.time = [];
                    for (let i=0; i<$scope.timesCount; i++){
                        obj.time.push("");
                    }
        ...
        $scope.addTime = function(){
            if ($scope.timesCount >= 4){
                let offset = 200*($scope.timesCount-3);
                let pixels = 1000+offset;
